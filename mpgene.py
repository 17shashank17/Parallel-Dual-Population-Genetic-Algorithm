...
def knapsack(weight, value, MAX, popSize, mut, maxGen, percent):
  ...
  while(not test(fitness, percent) and generation < maxGen):
    ...
    #pop=pool.apply(newPopulation,args=(pop,fitness,mut))
    #reserve_pop=pool.apply(newPopulation,args=(reserve_pop,reserve_fitness,mut))
    pool=mp.Pool(mp.cpu_count())
    fitness=pool.apply(getFitness,args=(pop,weight,value,MAX))
    pool.close()
    #fitness = getFitness(pop, weight, value, MAX)
    reserve_fitness=getFitness(reserve_pop,weight,value,MAX)
    #pool=mp.Pool(mp.cpu_count())
    #reserve_fitness=pool.apply(getFitness,args=(reserve_pop,weight,value,MAX))
    #pool.close()
    #pool=mp.Pool(mp.cpu_count())
    pop=crossBreeding(pop,reserve_pop,fitness,reserve_fitness)
    #pop=pool.apply(crossBreeding,args=(pop,reserve_pop,fitness,reserve_fitness))
    #pool.close()
...
'''def getFitness(chromosome,weight,value,MAX):
  sum_weight = MAX+1
  sum_value = 0
  while (sum_weight > MAX):
    sum_weight = 0
    ...
    for j in range(len(chromosome)):
      if chromosome[j] == 1:
        ...
        ones += [j]
    if sum_weight > MAX:
      chromosome[ones[random.randint(0, len(ones)-1)]] = 0
  return sum_weight'''
...
'''weight = [random.randint(5,20) for i in range(20)]
value = [random.randint(5,100) for i in range(20)]
print(weight)
print(value)'''
